...
export default function ContactPage() {
  return <>
    ...
    <section className="content-section"><div className="contact-layout">
      <aside className="contact-info"><span className="kicker">B2B ENQUIRIES</span><h2>Start with what you know.</h2><p>You do not need a complete technical specification to contact us. Basic dimensions and the shipment context are enough to begin.</p><div className="contact-details"><div className="contact-detail"><small>Email</small><strong>{siteConfig.email}</strong></div><div className="contact-detail"><small>Phone / WhatsApp</small><strong>{siteConfig.phone}</strong></div><div className="contact-detail"><small>Base</small><strong>{siteConfig.location}</strong></div><div className="contact-detail"><small>Service area</small><strong>{siteConfig.serviceArea}</strong></div></div></aside>
      <form className="quote-form" action={`mailto:${siteConfig.email}`} method="post" encType="text/plain"><div className="form-grid">
