...
export default function IndustriesPage() {
  return <>
    ...
    <section className="cards-page"><div className="industry-page-grid">{industries.map(([name, text], i) => <div className="industry-page-card" key={name}><span>{String(i + 1).padStart(2, "0")} · INDUSTRY</span><h3>{name}</h3><p>{text}</p></div>)}</div></section>
    <QuoteBand />
  </>;
}
