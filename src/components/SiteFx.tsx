...
/**
 * Site-wide pointer effects (progressive enhancement only).
 *
 * - Cards: sets --fx-x / --fx-y on the hovered card so the CSS spotlight
 *   follows the cursor. (Keep CARD_SELECTOR in sync with the spotlight rule
 *   in globals.css.)
 * - Photo panels: sets --fx-dx / --fx-dy (px from centre), --fx-rotate (3D
 *   tilt) and --fx-cx / --fx-cy (0–1000, used by the HUD coordinate readout).
 * - Home "packaging lab": feeds --mrx / --mry to its existing tilt rule.
 *
 * One passive pointermove listener, throttled to one update per frame. It does
 * nothing on touch devices or when the visitor prefers reduced motion, and
 * every page renders exactly the same without it.
 */
const CARD_SELECTOR = [
  ".exact-service-list li",
  ".exact-service-numbered li",
  ".exact-service-fact",
  ".why-card",
  ".industry-card",
  ".workflow-card",
  ".metric:not(.metric-accent)",
  ".company-route-card",
  ".principle-card",
  ".value-card",
  ".purpose-card",
  ...
  ".service-signal-grid > div",
  ".product-signal-grid > div",
  ".selection-factor-card",
  ".product-route-card",
  ".catalog-static-card",
  ".product-category-hub-card",
  ".wooden-subsection-card",
  ".wooden-requirement-card",
  ".wooden-spec-strip-inner > div",
  ".product-option-card",
...
const VISUAL_SELECTOR = [
  ".exact-service-hero-visual",
  ".page-hero-photo",
  ".product-main-image",
  ".gallery-photo-card",
  ".dark-feature-image-slot",
  ".material-visual-static",
  ".why-image-panel",
].join(", ");

const LAB_SELECTOR = ".hero-lab";
const VISUAL_PROPS = ["--fx-dx", "--fx-dy", "--fx-cx", "--fx-cy", "--fx-rotate"];
const LAB_PROPS = ["--mrx", "--mry"];
...
export function SiteFx() {
  useEffect(() => {
    ...
    let activeLab: HTMLElement | null = null;
    ...
    const resetLab = () => {
      if (!activeLab) return;
      LAB_PROPS.forEach((prop) => activeLab?.style.removeProperty(prop));
      activeLab = null;
    };
    ...
    const update = () => {
      ...
      const lab = target?.closest<HTMLElement>(LAB_SELECTOR) ?? null;
      ...
      if (activeLab && activeLab !== lab) resetLab();
      if (lab && labRect && labRect.width > 0 && labRect.height > 0) {
        const mx = clamp01((event.clientX - labRect.left) / labRect.width) * 2 - 1;
        const my = clamp01((event.clientY - labRect.top) / labRect.height) * 2 - 1;
        lab.style.setProperty("--mrx", mx.toFixed(3));
        lab.style.setProperty("--mry", my.toFixed(3));
        activeLab = lab;
      }
    };
